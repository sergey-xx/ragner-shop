...
from backend.constants import CODES_MAP
...
class Order(models.Model):
    ...
    tg_user = models.ForeignKey(TgUser, on_delete=models.CASCADE, verbose_name='TG USER')
    ...
    pubg_id = models.CharField(blank=True, null=True, max_length=50, verbose_name='PUBG id')
    ...
    def __str__(self):
        return (f"{self.id}.{self.data.get('price')} USDT "
    ...
    def user_str(self):
        ...
        codes = f'Code USED: {" ".join([code.code for code in self.uc_codes.all()])}\n' if self.uc_codes.all() else ''
        return (
            f'{completed}'
            f'{self.title}\n'
            f'{pubg_id}'
    ...
    def grab_code(self):
        ...
        codes = StockbleCode.objects.filter(amount=self.item.amount)[:(self.quantity - codes_count)]
        for code in codes:
            code.order = self
    ...
    def get_code_nominals(self):
        if not self.category == Item.Category.PUBG_UC:
    ...
    def grab_uc(self):
        ...
        for nom in nominals:
            ...
            if code:
                ...
                text = f'There is not available code {self.item.amount} for order {self.id}'
                logger.warning(text)
    ...
    def send_manager_notification(self, text, keyboard: str | None = None, kwargs: dict | None = None):
        if chat := self.item.chat:
            send_notification_task.delay(chat.tg_id, text, keyboard, kwargs={'id': self.id})
        else:
            logger.warning(f'There no chat for Item {self.item.value}')

    def cancel(self):
        if self.is_completed is not None:
            logger.error(f'Order {self.id} can`t be cancelled! Because has alreary have status `{self.status}`')
            return
    ...
    def status(self):
        ...
        return self.__class__.Status.COMPLETED if self.is_completed else self.__class__.Status.FAILED


class TopUp(models.Model):
    tg_user = models.ForeignKey(TgUser, on_delete=models.CASCADE, verbose_name='TG USER')
    amount = models.DecimalField(max_digits=10, decimal_places=3, verbose_name='Amount to topup')
    comission = models.DecimalField(blank=True, max_digits=4, decimal_places=3, verbose_name='Comission')
    to_pay = models.DecimalField(blank=True, max_digits=10, decimal_places=3, verbose_name='Total to pay')
