...
from codes.models import UcCode, Giftcard, StockbleCode
...
def import_codes_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            db_codes = [UcCode(amount=amount, code=code, activator=activator) for code in codes]
            UcCode.objects.bulk_create(db_codes, batch_size=1000,)
...
def import_giftcards_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            db_codes = [Giftcard(item=form.cleaned_data['item'], code=code) for code in form.cleaned_data['codes']]
            Giftcard.objects.bulk_create(db_codes, batch_size=1000,)
...
def import_stockblecode_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            db_codes = [
                StockbleCode(amount=form.cleaned_data['amount'], code=code) for code in form.cleaned_data['codes']
            ]
            StockbleCode.objects.bulk_create(db_codes, batch_size=1000,)
