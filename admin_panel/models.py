    ...
    def clean(self) -> None:
        ...
        if self.id:
            ...
            if self.attachments.exists() and len(self.text) > 1024:
                raise ValidationError(f'Text too long to send with attachment ({len(self.text)} of 1024)')
        return super().clean()
...
class Attachment(models.Model):
    ...
    mailing = models.ForeignKey(Mailing, on_delete=models.CASCADE, related_name='attachments', verbose_name='mailing')
    file_type = models.CharField(max_length=10, choices=FileType, verbose_name='Type of attachment')
